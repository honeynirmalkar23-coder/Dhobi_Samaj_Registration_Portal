...
declare
  ...
  v_storage_object_rows bigint := 0;
begin
  perform public.assert_admin();
  ...
  begin
    ...
    using (
      select 'applicant-photos'::text as bucket_id, r.applicant_photo_path as name
      from public.registrations r
      where nullif(btrim(coalesce(r.applicant_photo_path, '')), '') is not null

      union all

      select 'payment-proofs'::text as bucket_id, pp.storage_path as name
      from public.payment_proofs pp
      where nullif(btrim(coalesce(pp.storage_path, '')), '') is not null
    ) paths
    where so.bucket_id = paths.bucket_id
      and so.name = paths.name;

    get diagnostics v_storage_object_rows = row_count;
    ...
    perform public.insert_admin_audit_log(
      ...
      jsonb_build_object(
        ...
        'deletedStorageObjects', v_storage_object_rows,
